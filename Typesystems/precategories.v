...
Proof.
  ...
  Check (total_paths_equiv_fibr
   (fun x : precategory_objects C -> precategory_objects C' => forall a b : precategory_objects C, a --> b -> x a --> x b) (pr1 F)
   (pr1 G)) (base_paths F G p).

simpl.
  Print total2_paths2.
  set  (H3:= @total2_paths2 _ (fun p : pr1 (pr1 F) == pr1 (pr1 G) =>
          transportf
            (fun x : precategory_objects C -> precategory_objects C' =>
             (fun x0 : precategory_objects C -> precategory_objects C' => forall a b : precategory_objects C, a --> b -> x0 a --> x0 b) x)
   ...
   Check @total2_paths2.
   set (H3:= @total2_paths2 (pr1 F == pr1 G)  
...
Check @toforallpaths.
(*
orall (T : UU) (P : T -> UU) (f g : forall t : T, P t),
       f == g -> forall t : T, f t == g t
*)
...
Proof.
  intros T P f g h.
(*  set (H := homotinvweqweq (weqtoforallpaths _ f g)). *)
...
Lemma functor_eq_eq_if_object_eq_eq (C D : precategory)
    (F G : precategory_objects [C, D]) (p q : F == G) :
    base_paths (base_paths p) == pr1 (pr1 q) -> p == q.
...
(*
  ...
  simpl in *.
  destruct H6 as [HH HHH].
  simpl in *.
  rewrite HH.
  rewrite (pr2 H6).
  
  
  rewrite inv_from_iso.
  destruct F as [[F0 F1] HF].
  destruct G as [[G0 G1] HG].
  simpl in *.
  rewrite <- precategory_assoc.
  ...
  set (H2 := @isweqtoforallpaths _ _ (pr1 (pr1 F)) (pr1 (pr1 G))).
  Print isweq.
  unfold funextsec.
...
Proof.
  intro A.
  simpl in F, G.
  assert (H' : forall a, F a == G a).
  intro a.
  apply isotoid. assumption.
  apply (precategory_fun_iso_pointwise_if_iso _ _ _ _ A).
  apply A.
  assert (H2 : pr1 F == pr1 G).
  apply funextfun.

Lemma functor_precat_saturated_if_target_precat_is (C D : precategory)
    (H : is_saturated D) : is_saturated [C, D].
Proof.
  intros F G.
  simpl in *.
