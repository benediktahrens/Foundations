...
Proof.
  ...
  apply (total2_paths H').
   Check (pr2 (isofhlevel0weq X Y pX pY)).
...
(*
  case n.
  set (H:=isofhlevelweqb n (univalenceweq X Y)).
  apply H.
  assert (H' : isofhlevel n (X -> Y)).
  Check impred.
  apply impred.
  intro x. assumption.
  Print weq. About pr1.
...
Proof.
  exists (@pr1 (X == X') (fun w : X == X' => 
                   @identity (P X') (@transportf UU (fun x : UU => P x) X X' w pX) pX' )).
