...
(* The type of towers of hsets. *)

Section stower_def.

CoInductive stower := stowerconstr : forall T0 : hSet, ( T0 -> stower ) -> stower .

(* e.g. CoFixpoint consttower ( T : Type ) : tower := 
  towerconstr T ( fun t : T => consttower T ) . *)

Definition stowerbasefiber ( T : stower ) : total2 (fun T' : hSet => T' -> stower) := 
  match T with ( stowerconstr T' S' ) => 
        tpair (fun T' : hSet => ( T' -> stower )) T' S' end .  

Definition stowerbase ( T : stower ) : hSet := pr1 ( stowerbasefiber T ).  

Definition stoweroverbase { T : stower } ( t : stowerbase T ) : stower := 
...
Definition stowerstage ( n : nat ) ( T : stower ) : 
      total2 ( fun Tn : hSet => forall t : Tn , stower ) .
Proof. 
  intro n. 
  induction n . 
  intro T.  
  exact ( stowerbasefiber T ) . 

  intro T.
  assert (H : isaset 
          (total2 ( fun x : pr1 ( IHn T ) => stowerbase ( pr2 ( IHn T ) x )))).
  change isaset with (isofhlevel 2).
  apply isofhleveltotal2.
  apply (pr1 (IHn T)).
  intro x. apply (stowerbase (pr2 (IHn T) x)).
  exists (hSetpair (total2 ( fun x : pr1 ( IHn T ) => stowerbase ( pr2 ( IHn T ) x ))
               ) H).  
  intro x .
  exact (stoweroverbase (pr2 x)).
(*  destruct x as [ xn G ] . 
  exact ( toweroverbase G ) .   
*)
Defined.

Fixpoint stowerstage' (n : nat) : stower -> 
    total2 (fun Tn : hSet => forall t : Tn , stower) :=
 match n with
 | 0 => fun T => stowerbasefiber T
 | S n' => fun T => tpair (fun Tn : hSet => forall t : Tn , stower)
    (hSetpair
     (total2 (fun x : pr1 (stowerstage' n' T) => 
           stowerbase (pr2 (stowerstage' n' T) x)))
          (isofhleveltotal2 2 _ ((pr2 (pr1 (stowerstage' n' T)))) 
           (fun x => pr2 (stowerbase (pr2 (stowerstage' n' T) x))))) 
   (fun x => stoweroverbase (pr2 x))
 end.


Definition stowerfloor ( n : nat ) ( T : stower ) : hSet := pr1 (stowerstage n T).

Definition stowerfather {n : nat} {T : stower} (x : stowerfloor (S n) T) :
    stowerfloor n T := pr1 x.

Definition stowerover { n : nat } { T : stower } (G : stowerfloor n T) : stower := 
   pr2 (stowerstage n T) G.

Definition stowerfiber {n : nat} {T : stower} (G : stowerfloor n T) := 
   stowerbase (stowerover G). 

(*
Definition towerfatherofextension (n : nat) (T : tower) (G : towerfloor n T)
  (Y : towerfiber G) : paths (towerfather (tpair _ Y G)) G .
*)
Definition stowerfibertostowerfloor { n : nat } { T : stower } ( G : stowerfloor n T ) 
   ( G' : stowerfiber G ) : stowerfloor ( S n ) T := tpair _ G G' .

Lemma stowerfatherofextension (n : nat) (T : stower) (G : stowerfloor n T)
  (Y : stowerfiber G) : paths (stowerfather (stowerfibertostowerfloor G Y)) G .
Proof.
  intros.
  apply idpath.
Qed.
