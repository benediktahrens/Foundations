...
Definition yoneda_ob_functor_data (C : precategory) (c : ob C) :
...
Lemma is_functor_yoneda_functor_data (C : precategory) (c : ob C) :
...
Proof.
  ...
  apply idpath.
  set (H':= nat_trans_ax _ _ alpha  c a' f). simpl in H'.
  simpl in *.
  rewrite <- H'.
  clear H'. 
  unf. simpl.
